...
export function TableDetails({ themeClass, theme, data }) {
  ...
  return (
    <table className="table1 table">
      ...
      <thead>
        <tr>
          <th scope="col" className="th_size">
            {t('ELEMENT_RESULTS.result.ocurrenceNumber')}
          </th>
      ...
      <tbody>
        {data &&
          data.map((item, index) => (
            <>
              <tr key={index}>
                <td className="ama-typography-display-4 align-middle text-center">
                  {index + 1}
                </td>
                <td>
                  <dl className="text-start">
                    ...
                    <dt className="mb-2">
                      {t('ELEMENT_RESULTS.result.highlight_element')}
                    </dt>
                    <dd className="mb-4">
                      <div>
                        <div
                          style={{ display: 'block' }}
                          id={`showElement_${index + 1}`}
                        >
                          <ButtonsHighlight
                            handleHighlight={() =>
                              handleHighlightElement(index, item?.pointer)
                            }
