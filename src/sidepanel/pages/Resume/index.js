...
import {
  ...
  setCsvProcessedData,
  setCounter,
} from '../../store/slice/evaluationSlice';
...
export default function Resume({ setAllData, setEle }) {
  ...
  useEffect(() => {
    if (
      !reEvltd &&
      prevOriginalData.data &&
      prevDataProcess.metadata &&
      code.length != 0
    ) {
      ...
      <div className="report_container">
        ...
        {loadingProgress ? (
          ...
          <ButtonsActions
            reRequest={reRequest}
            downloadCSV={() =>
              downloadCSV(totalEvals, csvDataProcess, csvOriginalData, t)
            }
            downloadUploadableCSV={() =>
              downloadUploadableCSV(
                originalData.pagecode || 'html',
                csvOriginalData,
              )
            }
      ...
      {!loadingProgress && !error && (
        <>
          ...
          <section className="bg-white avaliation_container">
            <h2 className="avaliation_title mb-3">
              {t('RESULTS.results.title')}
            </h2>
            <TableComponent
